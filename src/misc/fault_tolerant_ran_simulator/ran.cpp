...
bool Ran::conn_check(int connIndex){
	return true;
	int err_code=0;
	Packet pk;
	pk.clear_pkt();
	...
	switch(connIndex){
		...
		case 1: 
			...
			err_code = mme_client_attach_1.snd(pk);
			break;
		case 2: 
		
		
			err_code = mme_client_attach_2.snd(pk);
			break;
		case 3: 
		
			
			err_code = mme_client_attach_3.snd(pk);
			break;
		case 4: 
		
		
			err_code = mme_client_attach_4.snd(pk);
			break;
		case 5: 
		
		
			err_code = mme_client_detach.snd(pk);
			break;
	}
...
void Ran::refresh_conn(int step) {
	...
	if(UE_BINDING == STATE_LESS){
		...
		switch(step){
			...
			case 1:
					...
					mme_client_attach_1.conn_refresh();
			break;
			
			
			case 2:			
					mme_client_attach_2.release();
					mme_client_attach_2.conn_refresh();
			break;
			
			
			case 3:			
					mme_client_attach_3.release();
					mme_client_attach_3.conn_refresh();
			break;
			
			
			case 4:			
					mme_client_attach_4.release();
					mme_client_attach_4.conn_refresh();
			break;
			
			
			case 5:			
					mme_client_detach.release();
					mme_client_detach.conn_refresh();
			break;	
...
bool Ran::initial_attach() {
	...
	err_code = mme_client_attach_1.snd(pkt);
	TRACE(cout << "ran_initialattach:" << " request sent for ran: " << ran_ctx.imsi << endl;)
	

	err_code = mme_client_attach_1.rcv(pkt);
	if (pkt.len <= 0) {cout<<"packet length zero"<<endl;
		return false;
	}
...
bool Ran::authenticate() {
	...
	if(err_code<0) {
		cout<<"FAILURE"<<ran_ctx.mme_s1ap_ue_id;
	return false;
	}
	if(UE_BINDING==STATE_LESS){
		err_code  = mme_client_attach_2.rcv(pkt);
	}	
	else if(UE_BINDING==SESSION_BASED){
		err_code = mme_client_attach_1.rcv(pkt);
	}
	...
	if (HMAC_ON) {
		...
		if (res == false) {
			...
			return false;g_utils.handle_type1_error(-1, "hmac error: ran_setsecurity");
		}
	}
	free(hmac_res);
	free(hmac_xres);
...
bool Ran::set_security(TrafficMonitor &traf_mon) {
	...
	if(UE_BINDING==STATE_LESS){
		err_code = mme_client_attach_3.snd(pkt);/*XCHANGE 3*/
	}	
	else if(UE_BINDING==SESSION_BASED){
		err_code = mme_client_attach_1.snd(pkt);/*XCHANGE 3*///cout<<"sess based xch3"<<endl;

	}else if(UE_BINDING==UE_BASED){
		err_code = mme_client_attach_1.snd(pkt);/*XCHANGE 3*/
	}
	...
	if(UE_BINDING==STATE_LESS){
			err_code = mme_client_attach_3.rcv(pkt);/*XCHANGE 3*/
	}	
	else if(UE_BINDING==SESSION_BASED){
			err_code = mme_client_attach_1.rcv(pkt);/*XCHANGE 3*///cout<<"sess based xch3"<<endl;

	}else if(UE_BINDING==UE_BASED){
			err_code = mme_client_attach_1.rcv(pkt);/*XCHANGE 3*/
	}
	if (pkt.len <= 0) {
		return false;
	}
	...
	if (HMAC_ON) {
		...
		if (res == false) {
			...
			return false;g_utils.handle_type1_error(-1, "hmac error: ran_setepssession");
		}
	}
	if (ENC_ON) {
...
bool Ran::set_eps_session(TrafficMonitor &traf_mon) {
	...
	if(UE_BINDING == STATE_LESS){
			err_code = mme_client_attach_4.snd(pkt);
	}	
	else if(UE_BINDING == SESSION_BASED){
			err_code = mme_client_attach_1.snd(pkt);//cout<<"sess based xch4"<<endl;
	...
	if(UE_BINDING==STATE_LESS){
			err_code = mme_client_attach_4.rcv(pkt);
	}	
	else if(UE_BINDING==SESSION_BASED){
			err_code = mme_client_attach_1.rcv(pkt);//cout<<"sess based xch4"<<endl;
	}
	else if(UE_BINDING==UE_BASED){
			err_code = mme_client_attach_1.rcv(pkt);
	}
	...
	if(err_code<0) cout<<"FAILURE"<<ran_ctx.imsi;
	return true;
}
...
bool Ran::detach() {
	...
	if(UE_BINDING==STATE_LESS){		
		err_code = mme_client_detach.snd(pkt);
	}	
	else if(UE_BINDING==SESSION_BASED){
		err_code = mme_client_detach.snd(pkt);
	}
	...
	if(UE_BINDING==STATE_LESS){		
		err_code = mme_client_detach.rcv(pkt);
	}	
	else if(UE_BINDING==SESSION_BASED){
		err_code = mme_client_detach.rcv(pkt);
	}
	...
	TRACE(cout << "ran_detach:" << " detach complete receivled from mme: " << pkt.len << ": " << ran_ctx.imsi << endl;)
	pkt.extract_s1ap_hdr();
	if (HMAC_ON) {
		res = g_integrity.hmac_check(pkt, ran_ctx.k_nas_int);
		if (res == false) {
			TRACE(cout << "ran_detach:" << " hmac detach failure: " << ran_ctx.imsi << endl;)
			return false;g_utils.handle_type1_error(-1, "hmac error: ran_detach");
		}
	}
	if(err_code<0) cout<<"FAILURE"<<ran_ctx.imsi;
	if (ENC_ON) {
